...
**多表联合（select*）实为两表各行的笛卡尔乘积，where语句的原理是笛卡尔乘积过滤**
**where的原理是先无条件笛卡尔乘积连接两表，之后删掉不符合条件的行**
**因此两个表连接，即笛卡尔乘积的结果是第三个集合**
```sql
...
## 3.2 子查询
**explain plan:执行计划，先对全表做扫描，再对from里做多次临时查询，最终得到记录**
**在形参中显示的表，即为执行查询的表，下方行是先执行的**
